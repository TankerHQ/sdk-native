...
class TankerConan(ConanFile):
    ...
    def requirements(self):
        ...
        self.requires("Boost/1.66.0-r9@tanker/stable", private=private, override=True)
        if self.options.with_ssl:
            self.requires("LibreSSL/2.6.3-r1@tanker/stable", private=private)
        self.requires("libsodium/1.0.16-r1@tanker/stable", private=private)
        self.requires("socket.io-client-cpp/1.6.1-r17@tanker/stable", private=private)
        self.requires("sqlpp11/0.56-r4@tanker/stable", private=private)
        self.requires("sqlpp11-connector-sqlite3/0.29-r7@tanker/stable", private=private)
        self.requires("tconcurrent/0.16@tanker/stable", private=private)
        self.requires("cppcodec/edf46ab@tanker/stable", private=private)
        self.requires("variant/1.3.0@tanker/stable", private=private)
        self.requires("gsl-lite/0.31.0@tanker/stable", private=private)
        self.requires("enum-flags/6df748a@tanker/stable", private=private)
        self.requires("mockaron/0.2@tanker/stable", private=private)

        self.requires("fmt/5.2.1@bincrafters/stable", private=private)
        self.requires("jsonformoderncpp/3.3.0@vthiery/stable", private=private)
        self.requires("optional-lite/2.3.0@nonstd-lite/stable", private=private)
    ...
    def build_requirements(self):
        ...
        self.build_requires("trompeloeil/29@tanker/stable")
        self.build_requires("docopt.cpp/0.6.2-r2@tanker/stable")

    def build(self):
        cross_building = tools.cross_building(self.settings)
        # develop is false when the package is used as a requirement.
        should_build_tests = self.develop
        should_run_tests = self.options.with_ssl and should_build_tests and self.should_test and not cross_building

        cmake = CMake(self)
        if self.options.sanitizer:
            cmake.definitions["CONAN_C_FLAGS"] = self.sanitizer_flag
            cmake.definitions["CONAN_CXX_FLAGS"] = self.sanitizer_flag
        cmake.definitions["BUILD_TESTS"] = should_build_tests
        cmake.definitions["BUILD_TANKER_TOOLS"] = should_build_tests
