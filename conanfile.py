...
class TankerConan(ConanFile):
    ...
    def requirements(self):
        ...
        self.requires("boost/1.78.0-r6", private=private)
        self.requires("libressl/3.5.3", private=private)
        self.requires("libcurl/7.80.0-r3", private=private)
        if self.options.with_sqlite:
            self.requires("sqlpp11/0.60-r4", private=private)
            self.requires("sqlpp11-connector-sqlite3/0.30-r4", private=private)
        ...
        self.requires("tconcurrent/0.40.0-r3", private=private)
        self.requires("date/3.0.0-r3", private=private)
    ...
    def build_requirements(self):
        ...
        if self.should_build_tests:
            self.test_requires("catch2-async/2.13.6-r4")
