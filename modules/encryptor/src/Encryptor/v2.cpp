...
constexpr auto overheadSize = versionSize + Crypto::AeadIv::arraySize + Crypto::Mac::arraySize;
...
std::uint64_t EncryptorV2::decryptedSize(gsl::span<std::uint8_t const> encryptedData)
{
...
EncryptCacheMetadata EncryptorV2::encryptSync(gsl::span<std::uint8_t> encryptedData,
                                              gsl::span<std::uint8_t const> clearData,
                                              Crypto::SymmetricKey const& key)
{
  encryptedData[0] = version();
  auto const iv = encryptedData.subspan(versionSize, Crypto::AeadIv::arraySize);
  auto const cipherText = encryptedData.subspan(versionSize + Crypto::AeadIv::arraySize);
  Crypto::randomFill(iv);
  auto const resourceId = Crypto::encryptAead(key, iv, cipherText, clearData, {});
  return EncryptCacheMetadata{SimpleResourceId(resourceId), key};
}

tc::cotask<EncryptCacheMetadata> EncryptorV2::encrypt(gsl::span<std::uint8_t> encryptedData,
                                                      gsl::span<std::uint8_t const> clearData)
{
  TC_RETURN(encryptSync(encryptedData, clearData, Crypto::makeSymmetricKey()));
}

tc::cotask<std::uint64_t> EncryptorV2::decrypt(gsl::span<std::uint8_t> decryptedData,
...
SimpleResourceId EncryptorV2::extractResourceId(gsl::span<std::uint8_t const> encryptedData)
{
  checkEncryptedFormat(encryptedData);
