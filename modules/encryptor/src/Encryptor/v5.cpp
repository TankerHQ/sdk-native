...
std::uint64_t EncryptorV5::decryptedSize(gsl::span<std::uint8_t const> encryptedData)
{
...
tc::cotask<EncryptCacheMetadata> EncryptorV5::encrypt(gsl::span<std::uint8_t> encryptedData,
                                                      gsl::span<std::uint8_t const> clearData,
                                                      SimpleResourceId const& resourceId,
                                                      Crypto::SymmetricKey const& key)
{
  encryptedData[0] = version();
  std::copy(resourceId.begin(), resourceId.end(), encryptedData.data() + versionSize);
...
tc::cotask<std::uint64_t> EncryptorV5::decrypt(gsl::span<std::uint8_t> decryptedData,
...
SimpleResourceId EncryptorV5::extractResourceId(gsl::span<std::uint8_t const> encryptedData)
{
  checkEncryptedFormat(encryptedData);
