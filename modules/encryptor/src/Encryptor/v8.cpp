...
std::uint64_t EncryptorV8::encryptedSize(std::uint64_t clearSize,
...
std::uint64_t EncryptorV8::decryptedSize(gsl::span<std::uint8_t const> encryptedData)
{
...
tc::cotask<EncryptCacheMetadata> EncryptorV8::encrypt(gsl::span<std::uint8_t> encryptedData,
...
tc::cotask<EncryptCacheMetadata> EncryptorV8::encrypt(gsl::span<std::uint8_t> encryptedData,
                                                      ...
                                                      std::uint32_t encryptedChunkSize)
{
  EncryptionStreamV8 encryptor(bufferViewToInputSource(clearData), resourceId, key, paddingStep, encryptedChunkSize);
...
tc::cotask<std::uint64_t> EncryptorV8::decrypt(gsl::span<std::uint8_t> decryptedData,
                                               Encryptor::ResourceKeyFinder const& keyFinder,
                                               gsl::span<std::uint8_t const> encryptedData)
{
  auto const initialSize = decryptedData.size();
  auto decryptor = TC_AWAIT(DecryptionStreamV8::create(bufferViewToInputSource(encryptedData), keyFinder));
...
SimpleResourceId EncryptorV8::extractResourceId(gsl::span<std::uint8_t const> encryptedData)
{
  Serialization::SerializedSource ss{encryptedData};
