...
std::uint64_t EncryptorV11::encryptedSize(
...
std::uint64_t EncryptorV11::decryptedSize(
    gsl::span<std::uint8_t const> encryptedData)
{
  ...
  auto const lastEncryptedChunkSize =
      ss.remaining_size() % header.encryptedChunkSize();
...
std::array<uint8_t, EncryptorV11::macDataSize> EncryptorV11::makeMacData(
...
SymmetricKey EncryptorV11::deriveSubkey(SymmetricKey const& sessionKey,
                                        SubkeySeed const& subkeySeed)
{
...
tc::cotask<EncryptCacheMetadata> EncryptorV11::encrypt(
    ...
    std::uint32_t encryptedChunkSize)
{
  EncryptionStreamV11 encryptor(bufferViewToInputSource(clearData),
                                sessionId,
                                sessionKey,
                                subkeySeed,
                                paddingStep,
    ...
    gsl::span<std::uint8_t const> encryptedData)
{
  ...
  auto decryptor = TC_AWAIT(DecryptionStreamV11::create(
...
CompositeResourceId EncryptorV11::extractResourceId(
    gsl::span<std::uint8_t const> encryptedData)
{
  Serialization::SerializedSource ss{encryptedData};
