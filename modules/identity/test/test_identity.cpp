...
auto const trustchainId = mgs::base64::decode<Trustchain::TrustchainId>(trustchainIdString);
auto const trustchainPrivateKey = mgs::base64::decode<Tanker::Crypto::PrivateSignatureKey>(trustchainPrivateKeyString);

auto const suserId = "b_eich"_uid;
auto const obfuscatedUserId = obfuscateUserId(suserId, trustchainId);

auto const userSecret =
    mgs::base64::decode<Tanker::Crypto::SymmetricKey>("7FSf/n0e76QT3s0DkvetRVVJhXZGEjOxj5EWAFexvjI=");

auto const publicEphemeralKey =
    mgs::base64::decode<Tanker::Crypto::PublicSignatureKey>("Xh3i0xDTpr3HXtB2Q517QKv3azNzXLLXMdJDTSH4bd4=");
auto const privateEphemeralKey = mgs::base64::decode<Tanker::Crypto::PrivateSignatureKey>(
    "jEDT4wQCc1DFwodXNPHFClndTPnFuFmXhBt+isKU4ZpeHeLTENOmvcde0HZDnXtAq/"
    "drM3Ncstcx0kNNIfht3g==");
auto const delegation_signature = mgs::base64::decode<Tanker::Crypto::Signature>(
...
auto const appSignatureKeyPair = Crypto::SignatureKeyPair{
    mgs::base64::decode<Tanker::Crypto::PublicSignatureKey>("W7QEQBu9FXcXIpOgq62tPwBiyFAbpT1rAruD0h/NrTA="),
...
auto const appEncryptionKeyPair = Crypto::EncryptionKeyPair{
    mgs::base64::decode<Tanker::Crypto::PublicEncryptionKey>("4QB5TWmvcBrgeyDDLhULINU6tbqAOEQ8v9pjDkPcybA="),
    mgs::base64::decode<Tanker::Crypto::PrivateEncryptionKey>("/2j4dI3r8PlvCN3uW4HhA5wBtMKOcACd38K6N0q+mFU="),
};

auto const ephemeralKeyPair = Crypto::SignatureKeyPair{publicEphemeralKey, privateEphemeralKey};

auto const delegation = Delegation{ephemeralKeyPair, obfuscatedUserId, delegation_signature};
...
auto const b64HashedEmail =
    mgs::base64::encode(Crypto::generichash(gsl::make_span(std::string(userEmail)).as_span<uint8_t const>()));

auto const appSignaturePublicKey =
    mgs::base64::decode<Tanker::Crypto::PublicSignatureKey>("W7QEQBu9FXcXIpOgq62tPwBiyFAbpT1rAruD0h/NrTA=");
auto const appSignaturePrivateKey = mgs::base64::decode<Tanker::Crypto::PrivateSignatureKey>(
    "UmnYuvdTaLYG0a+JaDpY6ojw4/2Ll8zsmramVC4fuqRbtARAG70Vdxcik6Crra0/"
    "AGLIUBulPWsCu4PSH82tMA==");
auto const appEncryptionPublicKey =
    mgs::base64::decode<Tanker::Crypto::PublicEncryptionKey>("/2j4dI3r8PlvCN3uW4HhA5wBtMKOcACd38K6N0q+mFU=");
auto const appEncryptionPrivateKey =
    mgs::base64::decode<Tanker::Crypto::PrivateEncryptionKey>("4QB5TWmvcBrgeyDDLhULINU6tbqAOEQ8v9pjDkPcybA=");

auto const phoneNumberAppSignaturePublicKey =
    mgs::base64::decode<Tanker::Crypto::PublicSignatureKey>("6Hoyz+k2gjrrpP6qfzQdBCiyzGEyj0V5lz2oURUkDDM=");
auto const phoneNumberAppSignaturePrivateKey = mgs::base64::decode<Tanker::Crypto::PrivateSignatureKey>(
...
auto const phoneNumberAppEncryptionPublicKey =
    mgs::base64::decode<Tanker::Crypto::PublicEncryptionKey>("00ydncd1LvJGCkYl9/Rs5ACLlvDcazj7Eg75z494QXU=");
auto const phoneNumberAppEncryptionPrivateKey =
    mgs::base64::decode<Tanker::Crypto::PrivateEncryptionKey>("SrDrcE/MnLxXqkZRHzrXoaIICJoxTGHmQgnR9eSOtQM=");

void checkUserSecret(Tanker::Crypto::SymmetricKey const& userSecret, UserId const& userId)
{
  auto const check =
      userSecretHash(gsl::make_span(userSecret).subspan(0, USER_SECRET_SIZE - 1).as_span<uint8_t const>(), userId);
  if (check[0] != userSecret[USER_SECRET_SIZE - 1])
    throw std::invalid_argument("bad user secret");
}

PublicIdentity createTestPermanentIdentity(Trustchain::TrustchainId const& trustchainId,
...
PublicIdentity createTestProvisionalIdentity(Trustchain::TrustchainId const& trustchainId)
{
  Trustchain::UserId userId;
  Crypto::randomFill(userId);
  auto const email = makeEmail();

  auto identity = Tanker::Identity::createProvisionalIdentity(trustchainId, email);
  return Identity::getPublicIdentity(identity);
}
}
...
TEST_CASE("generate Identity")
{
  ...
  {
    TANKER_CHECK_THROWS_WITH_CODE(createIdentity("trustchainID", "privateKey", ""_uid), Errc::InvalidUserId);
  }

  SECTION("should throw given empty trustchainId")
  {
    TANKER_CHECK_THROWS_WITH_CODE(createIdentity("", "privateKey", "userId"_uid), Errc::InvalidTrustchainId);
  }

  SECTION("should throw given empty privateKey")
  {
    TANKER_CHECK_THROWS_WITH_CODE(createIdentity("trustchainId", "", "userId"_uid), Errc::InvalidTrustchainPrivateKey);
  }

  SECTION("We can create an identity from strings")
  {
    CHECK_NOTHROW(createIdentity(trustchainIdString, trustchainPrivateKeyString, suserId));
  }
}
...
TEST_CASE("generate provisional Identity")
{
  SECTION("should throw given empty trustchainId")
  {
    TANKER_CHECK_THROWS_WITH_CODE(createProvisionalIdentity("", Email{userEmail}), Errc::InvalidTrustchainId);
  }

  SECTION("should throw given empty email")
  {
    TANKER_CHECK_THROWS_WITH_CODE(createProvisionalIdentity("trustchainId", Email{""}), Errc::InvalidEmail);
  }

  SECTION("We can create a provisional identity from strings")
  {
    CHECK_NOTHROW(createProvisionalIdentity(trustchainIdString, Email{userEmail}));
  }
}

TEST_CASE("serialization")
{
  SECTION("We can de/reserialize a secret permanent identity from a good string")
  {
    auto const identity = extract<SecretPermanentIdentity>(GOOD_SECRET_PERMANENT_IDENTITY);
  ...
  SECTION("We can de/reserialize a public permanent identity from a good string")
  {
    auto const publicIdentity = extract<PublicIdentity>(GOOD_PUBLIC_PERMANENT_IDENTITY);
    auto const publicPermanentIdentity = boost::variant2::get<PublicPermanentIdentity>(publicIdentity);
  ...
  SECTION(
      ...
      "identity")
  {
    TANKER_CHECK_THROWS_WITH_CODE(extract<PublicPermanentIdentity>(GOOD_SECRET_PERMANENT_IDENTITY), Errc::InvalidType);
  }
  ...
  SECTION(
      ...
      "string")
  {
    auto const identity = extract<SecretProvisionalIdentity>(GOOD_SECRET_PROVISIONAL_IDENTITY);
  ...
  SECTION(
      ...
      "good string")
  {
    auto const identity = extract<SecretProvisionalIdentity>(GOOD_SECRET_PHONE_NUMBER_PROVISIONAL_IDENTITY);

    CHECK(identity.trustchainId == trustchainId);
    CHECK(identity.value == phoneNumber);
    CHECK(identity.target == TargetType::PhoneNumber);
    CHECK(identity.appSignatureKeyPair.publicKey == phoneNumberAppSignaturePublicKey);
    CHECK(identity.appSignatureKeyPair.privateKey == phoneNumberAppSignaturePrivateKey);
    CHECK(identity.appEncryptionKeyPair.publicKey == phoneNumberAppEncryptionPublicKey);
    CHECK(identity.appEncryptionKeyPair.privateKey == phoneNumberAppEncryptionPrivateKey);
  ...
  {
    auto const identity = extract<PublicProvisionalIdentity>(GOOD_OLD_PUBLIC_PROVISIONAL_IDENTITY);
  ...
  {
    auto const identity = extract<PublicProvisionalIdentity>(GOOD_PUBLIC_PROVISIONAL_IDENTITY);
    auto hashedEmail =
        mgs::base64::encode(Crypto::generichash(gsl::make_span(userEmail).as_span<std::uint8_t const>()));
  ...
  {
    auto const identity = extract<PublicProvisionalIdentity>(GOOD_PUBLIC_PHONE_NUMBER_PROVISIONAL_IDENTITY);
    ...
    auto const hashedPhoneNumber = mgs::base64::encode(Crypto::generichash(buffer));
...
{
  TANKER_CHECK_THROWS_WITH_CODE(extract<PublicProvisionalIdentity>(GOOD_SECRET_PROVISIONAL_IDENTITY),
                                Errc::InvalidType);
}
...
TEST_CASE("getPublicIdentity")
{
  ...
  {
    auto const identityStr = createIdentity(trustchainIdString, trustchainPrivateKeyString, "alice"_uid);
  ...
  {
    auto const b64PublicIdentity = getPublicIdentity(GOOD_SECRET_PROVISIONAL_IDENTITY);
...
{
  CHECK_NOTHROW(checkUserSecret(generateUserSecret(obfuscatedUserId), obfuscatedUserId));
}
...
TEST_CASE("ensure public identity in trustchain")
{
  ...
  SECTION("throws with invalid identities")
  {
    ...
    auto identity = createTestPermanentIdentity(trustchainId, trustchainPrivateKey);
    TANKER_CHECK_THROWS_WITH_CODE(ensureIdentitiesInTrustchain({identity}, otherTrustchainId),
  ...
  SECTION("throws with a mix of valid and invalid identities")
  {
    ...
    auto validIdentity = createTestPermanentIdentity(otherTrustchainId, kp.privateKey);
    auto validProvisionalIdentity = createTestProvisionalIdentity(otherTrustchainId);
    auto invalidIdentity = createTestPermanentIdentity(trustchainId, trustchainPrivateKey);
    auto invalidProvisionalIdentity = createTestProvisionalIdentity(trustchainId);

    TANKER_CHECK_THROWS_WITH_CODE(
        ensureIdentitiesInTrustchain(
            {validIdentity, invalidIdentity, validProvisionalIdentity, invalidProvisionalIdentity}, otherTrustchainId),
        Errors::Errc::InvalidArgument);
  }
}
