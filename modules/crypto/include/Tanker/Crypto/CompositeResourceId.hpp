...
constexpr auto compositeSize = compositeTypeSize + SimpleResourceId::arraySize + SimpleResourceId::arraySize;
}
...
class CompositeResourceId : public BasicCryptographicType<CompositeResourceId, details::compositeSize>
{
  using base_t::base_t;

public:
  static CompositeResourceId newTransparentSessionId(SimpleResourceId const& sessionId,
                                                     SimpleResourceId const& individualResourceId)
  {
    CompositeResourceId id;
    id[0] = transparentSessionType();
    std::copy(sessionId.begin(), sessionId.end(), id.begin() + details::compositeTypeSize);
    std::copy(individualResourceId.begin(),
  ...
  {
    auto first = begin() + details::compositeTypeSize + SimpleResourceId::arraySize;
    auto last = first + SimpleResourceId::arraySize;
    return SimpleResourceId(first, last);
  };
};
