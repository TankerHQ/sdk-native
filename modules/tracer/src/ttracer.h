...
TRACEPOINT_ENUM(
    ...
    coro_state,
    TP_ENUM_VALUES(
        ctf_enum_value("Begin", Tanker::Tracer::CoroState::Begin)
...
TRACEPOINT_EVENT(
    ...
    TP_FIELDS(ctf_enum(ttracer, coro_state, int, state, pstate)
                  ctf_integer_hex(void*, coro_id, id)
                      ctf_integer_hex(void*, coro_stack, pstack)
                          ctf_enum(ttracer, coro_type, int, type, ptype)
                              ctf_string(msg, cmsg)))
...
TRACEPOINT_EVENT(
    ...
    TP_ARGS(void*,
            ...
            char const*,
            cmsg),
    TP_FIELDS(ctf_integer_hex(void*, coro_id, id)
                  ctf_integer_hex(void*, coro_stack, pstack)
                      ctf_float(double, coro_ts, duration)
                          ctf_enum(ttracer, coro_type, int, type, ptype)
                              ctf_string(msg, cmsg)))
TRACEPOINT_EVENT(ttracer,
                 func_beacon,
                 TP_ARGS(Tanker::Tracer::CoroType,
                         ptype,
                         Tanker::Tracer::CoroState,
                         pstate,
                         char const*,
