...
#define TANKER_TRUSTCHAIN_ACTIONS_USER_GROUP_REMOVAL_ATTRIBUTES \
  (groupId, GroupId), (membersToRemove, std::vector<UserId>),   \
      (provisionalMembersToRemove, std::vector<ProvisionalUserId>), (selfSignatureWithCurrentKey, Crypto::Signature)

class UserGroupRemoval
{
public:
  TANKER_IMMUTABLE_ACTION_IMPLEMENTATION(UserGroupRemoval, TANKER_TRUSTCHAIN_ACTIONS_USER_GROUP_REMOVAL_ATTRIBUTES)

public:
  UserGroupRemoval(TrustchainId const& trustchainId,
...
private:
  friend void from_serialized(Serialization::SerializedSource&, UserGroupRemoval&);
};
