...
#define TANKER_TRUSTCHAIN_ACTIONS_PROVISIONAL_IDENTITY_CLAIM_ATTRIBUTES                                        \
  (userId, UserId), (appSignaturePublicKey, Crypto::PublicSignatureKey),                                       \
      (tankerSignaturePublicKey, Crypto::PublicSignatureKey), (authorSignatureByAppKey, Crypto::Signature),    \
      (authorSignatureByTankerKey, Crypto::Signature), (userPublicEncryptionKey, Crypto::PublicEncryptionKey), \
...
public:
  ...
  class SealedPrivateEncryptionKeys
    : public Crypto::BasicCryptographicType<SealedPrivateEncryptionKeys,
                                            2 * Crypto::PrivateEncryptionKey::arraySize + crypto_box_SEALBYTES>
  {
    using base_t::base_t;
  };

  TANKER_IMMUTABLE_ACTION_IMPLEMENTATION(ProvisionalIdentityClaim,
                                         TANKER_TRUSTCHAIN_ACTIONS_PROVISIONAL_IDENTITY_CLAIM_ATTRIBUTES)

public:
  ProvisionalIdentityClaim(TrustchainId const& trustchainId,
                           UserId const& userId,
...
private:
  friend void from_serialized(Serialization::SerializedSource&, ProvisionalIdentityClaim&);
};
