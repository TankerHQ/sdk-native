...
#define TANKER_TRUSTCHAIN_ACTIONS_USER_GROUP_ADDITION2_ATTRIBUTES                                       \
  (groupId, GroupId), (previousGroupBlockHash, Crypto::Hash), (members, std::vector<UserGroupMember2>), \
      (provisionalMembers, std::vector<UserGroupProvisionalMember2>), (selfSignature, Crypto::Signature)
...
public:
  ...
  TANKER_IMMUTABLE_ACTION_IMPLEMENTATION(UserGroupAddition2, TANKER_TRUSTCHAIN_ACTIONS_USER_GROUP_ADDITION2_ATTRIBUTES)

public:
  UserGroupAddition2(TrustchainId const& trustchainId,
...
private:
  friend void from_serialized(Serialization::SerializedSource&, UserGroupAddition2&);
};
