...
#define TANKER_DETAIL_DEFINE_VARIANT_GETTER(unused1, unused2, elem)                                                 \
  ...
  {                                                                                                                 \
    return this->visit([](auto const& val) -> decltype(auto) { return val.TANKER_DETAIL_PARAMETER_NAME(elem)(); }); \
  }
...
public:                                                                                                              \
  name() = default;                                                                                                  \
                                                                                                                     \
  template <typename Alternative, typename = std::enable_if_t<std::is_constructible<variant_t, Alternative>::value>> \
  ...
  {                                                                                                                  \
    return boost::variant2::visit(std::forward<Callable>(c), _variant);                                              \
                                                                                                                     ...
                                                                                                                     \
  BOOST_PP_SEQ_FOR_EACH(TANKER_DETAIL_DEFINE_VARIANT_GETTER, BOOST_PP_EMPTY(), list)                                 \
                                                                                                                     \
...
#define TANKER_TRUSTCHAIN_ACTION_VARIANT_IMPLEMENTATION(name, types_tuple, ...) \
  TANKER_DETAIL_DEFINE_ACTION_VARIANT(name, types_tuple, BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__))

#define TANKER_TRUSTCHAIN_ACTION_VARIANT_IMPLEMENTATION_ZERO(name, types_tuple) \
