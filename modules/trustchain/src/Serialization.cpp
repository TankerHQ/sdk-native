...
Actions::Nature getBlockNature(gsl::span<std::uint8_t const> block)
{
  ...
  if (block[0] != blockVersion)
    throw Errors::formatEx(Errc::InvalidBlockVersion, "unsupported block version: {}", block[0]);
...
void deserializeBlockVersion(Serialization::SerializedSource& ss)
{
  ...
  {
    throw Errors::formatEx(Errc::InvalidBlockVersion, "unsupported block version: {}", version);
  }
}
...
void deserializeBlockNature(Serialization::SerializedSource& ss, Actions::Nature expected)
{
  ...
  if (nature != expected)
  {
    throw Errors::formatEx(Errc::InvalidBlockNature, "wrong block nature: expected {}, got {}", expected, nature);
  }
}
}
