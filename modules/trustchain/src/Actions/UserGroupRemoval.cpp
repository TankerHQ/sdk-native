...
{
UserGroupRemoval::UserGroupRemoval(
  ...
  : _trustchainId(trustchainId),
    ...
    _selfSignatureWithCurrentKey(
        Crypto::sign(signatureData(author), groupPrivateSignatureKey)),
    _author(author),
    _hash(computeHash()),
...
namespace
{
constexpr std::string_view userGroupRemovalSignaturePrefix =
    "UserGroupRemoval Signature";
}

std::vector<std::uint8_t> UserGroupRemoval::signatureData(
    DeviceId const& authorId) const
{
  std::vector<std::uint8_t> signatureData(
      userGroupRemovalSignaturePrefix.size() + DeviceId::arraySize +
      sizeof(uint32_t) + GroupId::arraySize +
      _membersToRemove.size() * UserId::arraySize + sizeof(uint32_t) +
      _provisionalMembersToRemove.size() *
          (Crypto::PublicSignatureKey::arraySize * 2));

  auto it = std::copy(userGroupRemovalSignaturePrefix.begin(),
                      userGroupRemovalSignaturePrefix.end(),
                      signatureData.data());
  ...
  it = Serialization::serialize<uint32_t>(it,
                                          _provisionalMembersToRemove.size());
