...
namespace Actions
{
UserGroupAddition1::UserGroupAddition1(TrustchainId const& trustchainId,
...
std::vector<std::uint8_t> UserGroupAddition1::signatureData() const
{
  ...
  it = std::copy(_previousGroupBlockHash.begin(), _previousGroupBlockHash.end(), it);
...
TANKER_TRUSTCHAIN_ACTION_DEFINE_METHODS(UserGroupAddition1, TANKER_TRUSTCHAIN_ACTIONS_USER_GROUP_ADDITION1_ATTRIBUTES)
}
}
}
