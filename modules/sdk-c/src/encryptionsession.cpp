...
CTANKER_EXPORT tanker_future_t* tanker_encryption_session_open(tanker_t* ctanker,
  ...
  return makeFuture(
      tc::sync([&] {
        ...
        return tanker->makeEncryptionSession(
            spublicIdentities, sgroupIds, static_cast<Core::ShareWithSelf>(shareWithSelf), paddingStepOpt);
      })
          .unwrap()
...
CTANKER_EXPORT tanker_future_t* tanker_encryption_session_close(tanker_encryption_session_t* csession)
{
  auto const session = reinterpret_cast<EncryptionSession*>(csession);
  return makeFuture(tc::async([=] { delete session; }));
}

CTANKER_EXPORT uint64_t tanker_encryption_session_encrypted_size(tanker_encryption_session_t* csession,
                                                                 uint64_t clearSize)
{
  auto const session = reinterpret_cast<EncryptionSession*>(csession);
...
CTANKER_EXPORT tanker_expected_t* tanker_encryption_session_get_resource_id(tanker_encryption_session_t* csession)
{
  auto const session = reinterpret_cast<EncryptionSession*>(csession);
  auto resourceId = mgs::base64::encode<SResourceId>(session->resourceId());
  return makeFuture(tc::make_ready_future(static_cast<void*>(duplicateString(resourceId.string()))));
}

CTANKER_EXPORT tanker_future_t* tanker_encryption_session_encrypt(tanker_encryption_session_t* csession,
                                                                  ...
                                                                  uint64_t data_size)
{
  ...
  return makeFuture(session->canceler()->run([&]() mutable {
    return tc::async_resumable([=]() -> tc::cotask<void> {
      auto const encryptedSpan = gsl::make_span(encrypted_data, session->encryptedSize(data_size));
      TC_AWAIT(session->encrypt(encryptedSpan, gsl::make_span(data, data_size)));
    });
  }));
}

tanker_future_t* tanker_encryption_session_stream_encrypt(tanker_encryption_session_t* csession,
                                                          ...
                                                          void* additional_data)
{
  ...
  return makeFuture(tc::sync([&] {
    ...
    auto [encryptor, resourceId] = session->makeEncryptionStream(std::move(wrappedCb));
