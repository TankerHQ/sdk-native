                                     ...
                                     uint64_t nb_public_identities_to_add)
{
  return makeFuture(tc::sync([&] {
                      auto const members = to_vector<SPublicIdentity>(
                          public_identities_to_add, nb_public_identities_to_add, "public_identities");
                      ...
                      return tanker->createGroup(members);
                        ...
                        .and_then(tc::get_synchronous_executor(),
                                  [](auto const& id) { return static_cast<void*>(duplicateString(id.string())); }));
}
                                             ...
                                             uint64_t nb_public_identities_to_remove)
{
  return makeFuture(tc::sync([&] {
                      ...
                      auto const public_identities_to_remove_vec = to_vector<SPublicIdentity>(
                          public_identities_to_remove, nb_public_identities_to_remove, "users_to_remove");

                      return tanker->updateGroupMembers(
                          SGroupId{group_id}, public_identities_to_add_vec, public_identities_to_remove_vec);
                    }).unwrap());
}
