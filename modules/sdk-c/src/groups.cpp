...
{
  auto const tanker = reinterpret_cast<AsyncCore*>(ctanker);
  auto const users_to_add_vec = to_vector<SUserId>(users_to_add, nb_users_to_add);

  return makeFuture(tanker->updateGroupMembers(SGroupId{group_id}, users_to_add_vec));
}
