...
private:
  ...
  static std::string formatError(std::error_code ec, std::string const& message);
};
...
TANKER_WARN_UNUSED_RESULT auto formatEx(Code c, fmt::string_view format, Args const&... args)
    -> std::enable_if_t<std::is_error_code_enum<Code>::value, Exception>
{
  return formatEx(static_cast<std::error_code>(c), format, fmt::make_format_args(args...));
}

template <typename... Args>
TANKER_WARN_UNUSED_RESULT Exception formatEx(Errc c, fmt::string_view format, Args const&... args)
{
