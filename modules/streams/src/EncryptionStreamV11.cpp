...
{
EncryptionStreamV11::EncryptionStreamV11(InputSource cb,
  ...
  : EncryptionStreamV11(std::move(cb), sessionId, sessionKey, getRandom<SubkeySeed>(), padding, encryptedChunkSize)
{
}
...
EncryptionStreamV11::EncryptionStreamV11(InputSource cb,
  ...
  : EncryptionStream(std::move(cb),
                     CompositeResourceId::newTransparentSessionId(sessionId, SimpleResourceId{subkeySeed}),
                     sessionKey,
                     encryptedChunkSize),
    _associatedData(EncryptorV11::makeMacData(sessionId, subkeySeed, encryptedChunkSize)),
...
tc::cotask<void> EncryptionStreamV11::encryptChunk()
{
  ...
  if (_paddingLeftToAdd)
  {
    ...
    paddedInputBuf = std::vector<uint8_t>(EncryptorV11::paddingSizeSize + paddingSize, 0);
  ...
  else
  {
    ...
    {
      auto const totalClearSize = _chunkIndex * clearChunkSize + clearData.size();
      _paddingLeftToAdd = Padding::paddedFromClearSize(totalClearSize, _paddingStep) - totalClearSize - 1;
      paddingSize = std::min<std::int64_t>(clearChunkSize - clearData.size(), *_paddingLeftToAdd);
      _paddingLeftToAdd = *_paddingLeftToAdd - paddingSize;
    }

    paddedInputBuf.resize(EncryptorV11::paddingSizeSize + paddingSize + clearData.size());
    std::copy(clearData.begin(), clearData.end(), paddedInputBuf.begin() + EncryptorV11::paddingSizeSize + paddingSize);
  }
  ...
  if (_paddingLeftToAdd && *_paddingLeftToAdd == 0 && paddedInputBuf.size() < _encryptedChunkSize - Mac::arraySize)
    endOutputStream();
}
}
