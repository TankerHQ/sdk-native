...
{
EncryptionStreamV4::EncryptionStreamV4(InputSource cb, std::uint32_t encryptedChunkSize)
  : EncryptionStreamV4(
        std::move(cb), Crypto::getRandom<Crypto::SimpleResourceId>(), Crypto::makeSymmetricKey(), encryptedChunkSize)
{
}
...
{
  auto const clearInput = TC_AWAIT(readInputSource(_encryptedChunkSize - overhead));
  auto output = prepareWrite(Header::serializedSize + Crypto::encryptedSize(clearInput.size()));

  Header const header(4u, _encryptedChunkSize, _resourceId, Crypto::getRandom<Crypto::AeadIv>());
