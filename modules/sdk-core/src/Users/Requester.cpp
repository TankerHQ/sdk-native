...
namespace
{
std::vector<Trustchain::UserAction> fromBlocksToUserActions(gsl::span<std::string const> blocks)
{
  return blocks |
         ranges::views::transform(ranges::compose(&Trustchain::deserializeUserAction, mgs::base64::lazy_decode())) |
         ranges::to<std::vector>;
}

std::vector<Trustchain::KeyPublishAction> fromBlocksToKeyPublishActions(gsl::span<std::string const> blocks)
{
  return blocks |
         ranges::views::transform(
             ranges::compose(&Trustchain::deserializeKeyPublishAction, mgs::base64::lazy_decode())) |
         ranges::to<std::vector>;
}
...
tc::cotask<Requester::GetResult> Requester::getUsersImpl(nlohmann::json const& query)
{
  auto url = _httpClient->makeUrl("user-histories", query);
  auto const response = TC_AWAIT(_httpClient->asyncGet(url)).value();
  auto rootBlock = Serialization::deserialize<Trustchain::Actions::TrustchainCreation>(
      mgs::base64::decode(response.at("root").get<std::string>()));
  TC_RETURN((GetResult{std::move(rootBlock),
...
tc::cotask<std::vector<Trustchain::KeyPublishAction>> Requester::getKeyPublishes(
    gsl::span<Crypto::SimpleResourceId const> resourceIds)
{
  auto const query =
      nlohmann::json{{"resource_ids[]", resourceIds | ranges::views::transform(mgs::base64url_nopad::lazy_encode())}};
  auto url = _httpClient->makeUrl("resource-keys", query);
  auto const response = TC_AWAIT(_httpClient->asyncGet(url)).value();
  TC_RETURN(fromBlocksToKeyPublishActions(response.at("resource_keys").get<std::vector<std::string>>()));
}
  ...
  auto const response =
      TC_AWAIT(
          _httpClient->asyncPost(
              ...
              {{"key_publishes_to_user", base64KeyPublishActions(actions.keyPublishesToUsers)},
               {"key_publishes_to_user_group", base64KeyPublishActions(actions.keyPublishesToUserGroups)},
               {"key_publishes_to_provisional_user", base64KeyPublishActions(actions.keyPublishesToProvisionalUsers)}}))
          .value();
}
    ...
    Trustchain::UserId const& userId, Crypto::PublicSignatureKey const& ghostDevicePublicSignatureKey)
{
  ...
  auto query = nlohmann::json{
      {"ghost_device_public_signature_key", mgs::base64url_nopad::encode(ghostDevicePublicSignatureKey)}};
  auto url = _httpClient->makeUrl(fmt::format("users/{userId:#S}/encryption-key", "userId"_a = userId), query);
  auto const res = TC_AWAIT(_httpClient->asyncUnauthGet(url)).value();
  TC_RETURN(
      (GetEncryptionKeyResult{res.at("encrypted_user_private_encryption_key").get<Crypto::SealedPrivateEncryptionKey>(),
...
Requester::getPublicProvisionalIdentities(gsl::span<HashedEmail const> hashedEmails)
{
  std::map<HashedEmail, std::pair<Crypto::PublicSignatureKey, Crypto::PublicEncryptionKey>> ret;
  if (hashedEmails.empty())
    TC_RETURN(ret);

  auto query = nlohmann::json{{"hashed_emails", hashedEmails | ranges::views::transform(mgs::base64::lazy_encode())}};
  ...
  for (auto const& elem : publicIdentitites.at("hashed_emails"))
  {
    ...
    auto const publicSignatureKey = elem.at("public_signature_key").get<Crypto::PublicSignatureKey>();
    auto const publicEncryptionKey = elem.at("public_encryption_key").get<Crypto::PublicEncryptionKey>();
...
Requester::getPublicProvisionalIdentities(gsl::span<HashedPhoneNumber const> hashedPhoneNumbers)
{
  std::map<HashedPhoneNumber, std::pair<Crypto::PublicSignatureKey, Crypto::PublicEncryptionKey>> ret;
  if (hashedPhoneNumbers.empty())
    TC_RETURN(ret);

  auto query = nlohmann::json{
      {"hashed_phone_numbers", hashedPhoneNumbers | ranges::views::transform(mgs::base64::lazy_encode())}};
  auto url = _httpClient->makeUrl("public-provisional-identities");
  auto const result = TC_AWAIT(_httpClient->asyncPost(url, query)).value();
  auto const publicIdentitites = result.at("public_provisional_identities");
