    ...
    std::vector<Crypto::PublicEncryptionKey> const& recipientUserKeys)
{
  ...
  for (auto const& keyResource : resourceKeys)
    for (auto const& recipientKey : recipientUserKeys)
      out.push_back(
          makeKeyPublishToUser(trustchainId, deviceId, signatureKey, recipientKey, keyResource.id, keyResource.key));
  return out;
}

std::vector<Trustchain::Actions::KeyPublishToProvisionalUser> generateShareBlocksToProvisionalUsers(
    Trustchain::TrustchainId const& trustchainId,
    Trustchain::DeviceId const& deviceId,
    Crypto::PrivateSignatureKey const& signatureKey,
    ResourceKeys::KeysResult const& resourceKeys,
    std::vector<ProvisionalUsers::PublicUser> const& recipientProvisionalUserKeys)
{
  ...
  for (auto const& keyResource : resourceKeys)
    for (auto const& recipientKey : recipientProvisionalUserKeys)
      out.push_back(makeKeyPublishToProvisionalUser(
          trustchainId, deviceId, signatureKey, recipientKey, keyResource.id, keyResource.key));
  return out;
}

std::vector<Trustchain::Actions::KeyPublishToUserGroup> generateShareBlocksToGroups(
    Trustchain::TrustchainId const& trustchainId,
    Trustchain::DeviceId const& deviceId,
    Crypto::PrivateSignatureKey const& signatureKey,
    ResourceKeys::KeysResult const& resourceKeys,
    std::vector<Crypto::PublicEncryptionKey> const& recipientUserKeys)
{
  ...
  for (auto const& keyResource : resourceKeys)
    for (auto const& recipientKey : recipientUserKeys)
      out.push_back(
          makeKeyPublishToGroup(trustchainId, deviceId, signatureKey, recipientKey, keyResource.id, keyResource.key));
  return out;
}

void handleNotFound(std::vector<SPublicIdentity> const& spublicIdentities,
                    ...
                    std::vector<Trustchain::GroupId> const& groupsNotFound)
{
  ...
  {
    auto const clearPublicIdentities = mapIdentitiesToStrings(usersNotFound, spublicIdentities, publicIdentities);
...
KeyRecipients toKeyRecipients(std::vector<Users::User> const& users,
                              ...
                              std::vector<Crypto::PublicEncryptionKey> const& groupEncryptionKeys)
{
  ...
  for (auto const& user : users)
  {
    if (!user.userKey())
    {
      throw Errors::AssertionError("sharing to users without user key is not supported anymore");
    }
...
{
  auto const encryptedKey = Crypto::sealEncrypt(resourceKey, recipientPublicEncryptionKey);

  return Users::createKeyPublishToUserAction(
      trustchainId, deviceId, signatureKey, encryptedKey, resourceId, recipientPublicEncryptionKey);
}
...
Trustchain::Actions::KeyPublishToUserGroup makeKeyPublishToGroup(
    ...
    Crypto::SymmetricKey const& resourceKey)
{
  auto const encryptedKey = Crypto::sealEncrypt<Crypto::SealedSymmetricKey>(resourceKey, recipientPublicEncryptionKey);

  return Groups::createKeyPublishToGroupAction(
      encryptedKey, resourceId, recipientPublicEncryptionKey, trustchainId, deviceId, signatureKey);
}
...
Trustchain::Actions::KeyPublishToProvisionalUser makeKeyPublishToProvisionalUser(
    ...
    Crypto::SymmetricKey const& resourceKey)
{
  auto const encryptedKeyOnce = Crypto::sealEncrypt(resourceKey, recipientProvisionalUser.appEncryptionPublicKey());
  auto const encryptedKeyTwice =
      Crypto::sealEncrypt(encryptedKeyOnce, recipientProvisionalUser.tankerEncryptionPublicKey());

  return Users::createKeyPublishToProvisionalUserAction(trustchainId,
                                                        deviceId,
                                                        signatureKey,
                                                ...
                                                std::vector<SGroupId> sgroupIds)
{
  ...
  auto const groupIds = sgroupIds | ranges::views::transform([](auto&& sgroupId) {
                          return decodeArgument<mgs::base64, Trustchain::GroupId>(sgroupId.string(), "group id");
                        }) |
  ...
  handleNotFound(spublicIdentities, publicIdentities, userResult.notFound, sgroupIds, groupIds, groupResult.notFound);

  auto const provisionalUsers =
      TC_AWAIT(userAccessor.pullProvisional(partitionedIdentities.publicProvisionalIdentities));

  TC_RETURN(toKeyRecipients(userResult.found, provisionalUsers, groupResult.found));
}
                                 ...
                                 KeyRecipients const& keyRecipients)
{
  auto keyPublishesToUsers =
      generateShareBlocksToUsers(trustchainId, deviceId, signatureKey, resourceKeys, keyRecipients.recipientUserKeys);
  auto keyPublishesToProvisionalUsers = generateShareBlocksToProvisionalUsers(
      trustchainId, deviceId, signatureKey, resourceKeys, keyRecipients.recipientProvisionalUserKeys);
  auto keyPublishesToGroups =
      generateShareBlocksToGroups(trustchainId, deviceId, signatureKey, resourceKeys, keyRecipients.recipientGroupKeys);

  return {std::move(keyPublishesToUsers), std::move(keyPublishesToGroups), std::move(keyPublishesToProvisionalUsers)};
}
                       ...
                       std::vector<SGroupId> const& groupIds)
{
  ...
  if ((publicIdentities.size() + groupIds.size()) * resourceKeys.size() > ShareLimit)
    throw formatEx(
        Errors::Errc::InvalidArgument, "cannot share with more than {} resource IDs x recipients at once", ShareLimit);

  auto const keyRecipients =
      TC_AWAIT(generateRecipientList(trustchainId, userAccessor, groupAccessor, publicIdentities, groupIds));

  auto const actions = generateShareBlocks(trustchainId, deviceId, signatureKey, resourceKeys, keyRecipients);
