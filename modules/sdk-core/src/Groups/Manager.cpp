...
namespace
{
ProcessedIdentities processIdentities(Trustchain::TrustchainId const& trustchainId,
                                      std::vector<SPublicIdentity> identities)
{
  ...
  ret.publicIdentities =
      ret.spublicIdentities | ranges::views::transform(extractPublicIdentity) | ranges::to<std::vector>;
...
tc::cotask<MembersToAdd> fetchFutureMembers(Users::IUserAccessor& userAccessor, ProcessedIdentities const& identities)
{
  ...
  if (!memberUsers.notFound.empty())
  {
    auto const notFoundIdentities =
        mapIdentitiesToStrings(memberUsers.notFound, identities.spublicIdentities, identities.publicIdentities);
    throw formatEx(Errc::InvalidArgument, "public identities not found: {:s}", fmt::join(notFoundIdentities, ", "));
  }
  auto const memberProvisionalUsers =
      TC_AWAIT(userAccessor.pullProvisional(identities.partitionedIdentities.publicProvisionalIdentities));
                                                 ...
                                                 ProcessedIdentities const& identities)
{
  ...
  ret.provisionalUsers =
      memberProvisionalUsers | ranges::views::transform(&ProvisionalUsers::PublicUser::id) | ranges::to<std::vector>;
    ...
    Crypto::PrivateSignatureKey const& deviceSignatureKey)
{
  ...
  auto groupMembers = generateGroupKeysForUsers2(groupEncryptionKeyPair.privateKey, memberUsers);
  auto groupProvisionalMembers =
      generateGroupKeysForProvisionalUsers3(groupEncryptionKeyPair.privateKey, memberProvisionalUsers);
...
{
  auto const processedIdentities = processIdentities(trustchainId, std::move(spublicIdentities));

  auto const members = TC_AWAIT(fetchFutureMembers(userAccessor, processedIdentities));
    ...
    Crypto::PrivateSignatureKey const& privateSignatureKey)
{
  ...
  if (groupSize == 0)
  {
    throw Exception(make_error_code(Errc::InvalidArgument), "must add at least one member to a group");
  }
  else if (groupSize > MAX_GROUP_SIZE)
  {
    throw formatEx(Errc::GroupTooBig,
  ...
  auto members = generateGroupKeysForUsers2(group.encryptionKeyPair.privateKey, memberUsers);
  auto provisionalMembers =
      generateGroupKeysForProvisionalUsers3(group.encryptionKeyPair.privateKey, memberProvisionalUsers);
                          ...
                          ProcessedIdentities const& identitiesToAdd)
{
  ...
  auto const usersBothAddedAndRemoved =
      ranges::views::set_intersection(usersToAdd, usersToRemove) | ranges::to<std::vector>;

  if (!usersBothAddedAndRemoved.empty())
  {
    auto const identitiesBothAddedAndRemoved = mapIdentitiesToStrings(
        usersBothAddedAndRemoved, identitiesToAdd.spublicIdentities, identitiesToAdd.publicIdentities);
    throw formatEx(
        Errc::InvalidArgument, "cannot both add and remove: {:s}", fmt::join(identitiesBothAddedAndRemoved, ", "));
  }
}
                   ...
                   std::vector<SPublicIdentity> spublicIdentitiesToRemove)
{
  ...
  auto const processedIdentitiesToAdd = processIdentities(trustchainId, std::move(spublicIdentitiesToAdd));
  auto const processedIdentitiesToRemove = processIdentities(trustchainId, std::move(spublicIdentitiesToRemove));

  if (!processedIdentitiesToAdd.spublicIdentities.empty())
  {
    membersToAdd = TC_AWAIT(fetchFutureMembers(userAccessor, processedIdentitiesToAdd));

    groupAddEntry = makeUserGroupAdditionAction(
        membersToAdd.users, membersToAdd.provisionalUsers, group, trustchainId, deviceId, privateSignatureKey);
  }

  if (!processedIdentitiesToRemove.spublicIdentities.empty())
  {
    membersToRemove = TC_AWAIT(fetchMembersToRemove(userAccessor, processedIdentitiesToRemove));

    groupRemoveEntry = makeUserGroupRemovalAction(
        membersToRemove.users, membersToRemove.provisionalUsers, group, trustchainId, deviceId, privateSignatureKey);
  }

  checkAddedAndRemoved(membersToAdd.users | ranges::views::transform(&Users::User::id) | ranges::to<std::vector>,
                       membersToRemove.users,
                       processedIdentitiesToAdd);

  checkAddedAndRemoved(membersToAdd.provisionalUsers | ranges::views::transform(&ProvisionalUsers::PublicUser::id) |
                           ranges::to<std::vector>,
                       membersToRemove.provisionalUsers,
                       processedIdentitiesToAdd);
  TC_RETURN(std::make_pair(std::move(groupAddEntry), std::move(groupRemoveEntry)));
}
}
                               ...
                               Crypto::PrivateSignatureKey const& privateSignatureKey)
{
  if (spublicIdentitiesToAdd.empty() && spublicIdentitiesToRemove.empty())
    throw formatEx(Errc::InvalidArgument, "no members to add or remove in updateMembers");

  auto const group = TC_AWAIT(groupAccessor.getInternalGroup(groupId));
  auto const [groupAddEntry, groupRemoveEntry] = TC_AWAIT(createGroupEntries(userAccessor,
                                                                             trustchainId,
                                                                             group,
                                                                             deviceId,
                                                                             privateSignatureKey,
