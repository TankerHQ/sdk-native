...
public:
  ...
  tc::cotask<std::optional<Crypto::PublicEncryptionKey>> userStatus(Trustchain::UserId const& userId) override;

  tc::cotask<void> setVerificationMethod(Trustchain::UserId const& userId,
                                         SetVerifMethodRequest const& request) override;

  tc::cotask<std::vector<std::uint8_t>> fetchVerificationKey(Trustchain::UserId const& userId,
                                                             RequestWithVerif const& verificationRequest) override;

  tc::cotask<std::vector<std::uint8_t>> fetchE2eVerificationKey(Trustchain::UserId const& userId,
                                                                RequestWithVerif const& verificationRequest) override;
  ...
  tc::cotask<Oidc::Challenge> getOidcChallenge(Trustchain::UserId const& userId, Oidc::Nonce const& nonce) override;

  tc::cotask<std::string> getSessionToken(Trustchain::UserId const& userId,
                                          gsl::span<uint8_t const> sessionCertificate,
                                          std::string nonce) override;

  tc::cotask<void> createUser(Trustchain::TrustchainId const& trustchainId,
  ...
  tc::cotask<void> createUserE2e(Trustchain::TrustchainId const& trustchainId,
                                 Trustchain::UserId const& userId,
                                 gsl::span<uint8_t const> userCreation,
                                 gsl::span<uint8_t const> firstDevice,
                                 RequestWithVerif const& verificationRequest,
  ...
  tc::cotask<void> enrollUser(Trustchain::TrustchainId const& trustchainId,
                              Trustchain::UserId const& userId,
                              gsl::span<uint8_t const> userCreation,
                              gsl::span<RequestWithVerif const> verificationRequests,
                              gsl::span<uint8_t const> encryptedVerificationKey) override;

  tc::cotask<void> createDevice(gsl::span<uint8_t const> deviceCreation) override;
