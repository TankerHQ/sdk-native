...
void GroupMatcher(Group const& resultGroup, Test::Group const& testGroup)
{
  ...
  CHECK(group->lastKeyRotationBlockHash == testGroup.lastKeyRotationBlockHash());
  ...
  else if constexpr (std::is_same_v<R, ExternalGroup>)
  {
    ...
    auto const decrypted = Crypto::sealDecrypt(group->encryptedPrivateSignatureKey, testGroup.currentEncKp());
...
TEST_CASE("Group V1")
{
  ...
  REQUIRE_CALL(aliceLocalUserAccessor, get()).LR_RETURN(aliceLocalUser).TIMES(AT_LEAST(0));

  SECTION("GroupCreation")
  {
    SECTION("handles creation of a group I am part of")
    {
      auto const testGroup = generator.makeGroupV1(alice.devices().front(), {alice});
      auto const resultGroup = AWAIT(GroupUpdater::applyUserGroupCreation(
          aliceLocalUserAccessor, aliceProvisionalUsersAccessor, makeEntries(testGroup).front()));
    ...
    SECTION("handles creation of a group I am *not* part of")
    {
      ...
      REQUIRE_CALL(aliceLocalUserAccessor, pull()).LR_RETURN(makeCoTask<Users::LocalUser const&>(aliceLocalUser));
      auto const resultGroup = AWAIT(GroupUpdater::applyUserGroupCreation(
          aliceLocalUserAccessor, aliceProvisionalUsersAccessor, makeEntries(group).front()));
  ...
  SECTION("GroupAddition")
  {
    SECTION("Alice sees Bob being added to her group")
    {
      ...
      auto const resultGroup = AWAIT(GroupUpdater::applyUserGroupAddition(
          aliceLocalUserAccessor, aliceProvisionalUsersAccessor, aliceGroup, makeEntries(aliceGroup).back()));
    ...
    SECTION("Alice sees herself being added to Bob's group")
    {
      ...
      auto const resultGroup = AWAIT(GroupUpdater::applyUserGroupAddition(aliceLocalUserAccessor,
    ...
    SECTION("Alice sees Charlie being added to Bob's group")
    {
      ...
      auto const resultGroup = AWAIT(GroupUpdater::applyUserGroupAddition(
          aliceLocalUserAccessor, aliceProvisionalUsersAccessor, static_cast<ExternalGroup>(bobGroup), entry));
...
TEST_CASE("Group V2")
{
  ...
  auto aliceLocalUserAccessor = LocalUserAccessorMock{};
  auto aliceProvisionalUsersAccessor = ProvisionalUsersAccessorMock{};
  auto const aliceProvisionalUser = generator.makeProvisionalUser("alice@tanker");
  auto const bob = generator.makeUser("bob");
  ...
  SECTION("GroupCreation")
  {
    ...
    SECTION("handles creation of a group I am part of")
    {
      auto const group = generator.makeGroupV2(alice.devices().front(), {alice});
      REQUIRE_CALL(aliceLocalUserAccessor, pullUserKeyPair(alice.userKeys().back().publicKey))
    ...
    SECTION("handles creation of a group I am *not* part of")
    {
      ...
      auto const resultGroup = AWAIT(GroupUpdater::applyUserGroupCreation(
          aliceLocalUserAccessor, aliceProvisionalUsersAccessor, makeEntries(bobGroup).front()));
    ...
    SECTION(
        ...
        "identity")
    {
      auto const bobGroup = generator.makeGroupV2(bob.devices().front(), {bob}, {aliceProvisionalUser});
      REQUIRE_CALL(aliceProvisionalUsersAccessor, findEncryptionKeysFromCache(trompeloeil::_, trompeloeil::_))
          .RETURN(makeCoTask(std::make_optional<ProvisionalUserKeys>(aliceProvisionalUser)));
  ...
  SECTION("GroupAddition")
  {
    ...
    SECTION("Alice sees herself being added to Bob's group as a provisional user")
    {
      auto bobGroup = generator.makeGroupV2(bob.devices()[0], {bob}, {});
      auto bobGroupUpdated = bobGroup;
